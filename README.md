...
export const taskPipeline = createPipeline<TaskContext>([
  ...
  step('pr-lifecycle',    handlePRLifecycle),      // PR 作成 + CI + マージ承認 + マージ実行
...
| `retry(from, reason)` | 指定 step まで巻き戻す | CI失敗 → `from: 'implementation'`<br>マージ失敗 → `from: 'pr-lifecycle'` |
...
// マージ実行失敗 → PRライフサイクルからやり直し（実装コードは変えない）
return { kind: 'retry', from: 'pr-lifecycle', reason: `マージ失敗: ${mergeError.message}` }
```
