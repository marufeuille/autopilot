...
3. コード変更（プログラムのソースコード）を伴う場合は、必ず対応するテストを作成すること。テストはユニットテストを基本とし、必要に応じて統合テストも書くこと。既存テストが壊れていないことも確認すること。ドキュメントのみの変更（README、設計書、設定ファイル等）にはテスト不要
4. コード変更を伴う場合は、実装完了前に既存のテストスイートを実行し、既存テストが壊れていないことを確認すること
...
function buildRetryPrompt(task: TaskFile, cwd: string, retryContext: RetryContext, useWorktree: boolean): string {
  ...
  prompt += `\n\n## 重要\n- プロジェクトのCLAUDE.mdとREADMEを読み、設計思想・規約に沿ってシンプルに実装すること。既存設計から逸脱した過剰な実装は避けること\n- コード変更（プログラムのソースコード）を伴う場合は、必ず対応するテストを作成すること。テストはユニットテストを基本とし、必要に応じて統合テストも書くこと。既存テストが壊れていないことも確認すること。ドキュメントのみの変更（README、設計書、設定ファイル等）にはテスト不要\n- コード変更を伴う場合は、実装完了前に既存のテストスイートを実行し、既存テストが壊れていないことを確認すること\n- PRの作成は自動で行われるため、\`gh pr create\` は実行しないこと\n\n上記の修正依頼を踏まえて、完了条件を再確認しながら修正してください。`;
