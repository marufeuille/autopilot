...
import { NotificationBackend, ApprovalResult, NotifyOptions, TaskFailureAction, QueueFailedAction, AcceptanceCheckResult, AcceptanceGateAction } from './types';
...
export class ResilientNotificationBackend implements NotificationBackend {
  ...
  async notifyUpdate(messageTs: string, message: string, storySlug?: string): Promise<void> {
    try {
      await this.withRetry(() => this.primary.notifyUpdate(messageTs, message, storySlug));
    } catch (error) {
      console.warn(
        `[resilient-notify] primary notifyUpdate failed after ${this.maxRetries + 1} attempts, falling back to local:`,
        error instanceof Error ? error.message : error,
      );
      await this.fallback.notifyUpdate(messageTs, message, storySlug);
    }
  }
  ...
  async requestQueueFailedAction(
    ...
    try {
      return await this.withRetry(() =>
        this.primary.requestQueueFailedAction(storySlug, message),
      );
    } catch (error) {
      console.warn(
        `[resilient-notify] primary requestQueueFailedAction failed after ${this.maxRetries + 1} attempts, falling back to local:`,
        error instanceof Error ? error.message : error,
      );
      return this.fallback.requestQueueFailedAction(storySlug, message);
    }
  }
  ...
  async requestAcceptanceGateAction(
    ...
    try {
      return await this.withRetry(() =>
        this.primary.requestAcceptanceGateAction(storySlug, checkResult),
      );
    } catch (error) {
      console.warn(
        `[resilient-notify] primary requestAcceptanceGateAction failed after ${this.maxRetries + 1} attempts, falling back to local:`,
        error instanceof Error ? error.message : error,
      );
      return this.fallback.requestAcceptanceGateAction(storySlug, checkResult);
    }
  }
